...
总结：
一个小小的工程游戏，有这么一些特色吧：
1. 数据懒加载：重新挂载仅重建目录树，不读reg文件。这么做的好处是，
“想查的东西立马可以确认有没有”，“想读的东西直接从磁盘读”，对于大容量储存性能友好
2. 给定容量， 通过数值计算划分区域，尽量把磁盘铺满。当然了，还是通过搜索分配更稳妥些

其实有很多想法没有实现，主要是读取部分：
1. 数据完全可以实现“增量写”， 实现方法类似于 casual write
2. ram完全可以实现cache机制， 否则面向小文件多次查询的场景，开销会很大

如果说有什么收获，就是：  
一开始就将系统设计好！！ 站在山上看问题， 会省很多事，尤其是需求变更的时候
